...
def train(model, epochs):
    ...
    entropy_loss = tf.keras.losses.BinaryCrossentropy()
    ...
    test_loss, test_accuracy = model.test(test_preprocessed, entropy_loss)
    test_losses.append(test_loss)
    test_accuracies.append(test_accuracy)

    # check how model performs on training data once before we begin
    train_loss, _ = model.test(train_preprocessed, entropy_loss)
    ...
    for epoch in range(epochs):
        ...
        for input,target in train_preprocessed:
            train_loss = model.train_step(input, target, entropy_loss, optimizer)
    ...
    val_loss, val_accuracy = model.test(val_preprocessed, entropy_loss)
